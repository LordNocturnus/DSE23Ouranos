...
class orbital_trajectory:
    ...
    def initial_manoeuvre_capsule(self,desired_periapsis,capsule_retrograde = False):
        ...
        #verif
        state = [semi_major_axis,eccentricity,0,4.17040775,0,true_anomaly]
...
#            atmospheric_encounter = count -1
    ...
    def get_capture_delta_v(self,desired_apoapsis):
        ...
        print('the captured state in keplerian coordinates is', captured_state_keplerian)
...
#            raise ValueError("The capsule posisiton is too high."),
        ...
        capturedstate = np.array([self.semi_major,self.eccentricity,0,output[0],0,output[1]])
        ...
        plt.plot(np.arange(stop=len(telemetry_distance )* 10, start = 0, step = 10)/3600, position_glider[:,0])
        plt.show()
        ...
        #print('fourier transform of angle is',np.fft.fft(telemetry_angle))
        #plt.plot(np.arange(stop=len(telemetry_distance )* 10, start = 0, step = 10)/3600,np.imag(np.fft.fft(telemetry_angle)))
        #plt.show()
        #print('Fourier transform of distance is',np.fft.fft(telemetry_distance))
        #plt.plot(np.arange(stop=len(telemetry_distance )* 10, start = 0, step = 10)/3600,np.imag(np.fft.fft(telemetry_distance)))
        #plt.show()
