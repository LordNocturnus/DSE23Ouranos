...
departure_date_lb = 11000 * constants.JULIAN_DAY
departure_date_ub = 14000 * constants.JULIAN_DAY
...
legs_tof_ub[2] = 500 * constants.JULIAN_DAY
# Jupiter fly-by
legs_tof_lb[3] = 400 * constants.JULIAN_DAY
legs_tof_ub[3] = 2000 * constants.JULIAN_DAY
# Uranus fly-by
legs_tof_lb[4] = 15*365 * constants.JULIAN_DAY
legs_tof_ub[4] = 25*365 * constants.JULIAN_DAY
...
repeats = 25
counter = 0
listofcorrectones = (np.zeros(repeats))
for k in range(samplesize):
    number_of_generations = 1
    dv_s = np.zeros(repeats) +1e99
    for j in range(repeats):
        ...
        if pop.champion_f < np.min(dv_s):
            dv_s[j] = pop.champion_f
        else:
            dv_s[j] = np.min(dv_s)
    for l in range(repeats):
        if dv_s[l] < 9940:
            listofcorrectones[l] += 1
    
    #print (counter,'out of',k+1,'found the correct solution!')
    print (k,'th analysis done!')

    #plt.plot(dv_s)
    #print (np.min(dv_s))
    #plt.show()
plt.plot(listofcorrectones/samplesize)
plt.show()
